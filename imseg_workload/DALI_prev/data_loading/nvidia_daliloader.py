...
def sharded_pipeline(files1, files2, device_id, shard_id, num_shards, patch_size, oversampling, batch_size, training=True):
 ...
 # Modified pipeline configuration
    pipe = Pipeline(
        ...
        num_threads=4,  # Increased to match CPU cores
        device_id=device_id     
    )
    ...
    with pipe:
        ...
        # Normalize types
        images = fn.cast(images, dtype=types.FLOAT, device="cpu")
        labels = fn.cast(labels, dtype=types.UINT8, device="cpu")
        images = images.gpu()
        labels = labels.gpu()
        ...
        images = fn.brightness_contrast(images, brightness=1.2, contrast=1.3, contrast_center=128)

        
        images = images + fn.random.normal(images, stddev=0.1)
        images = fn.gaussian_blur(images, window_size=31, sigma=2.5)
        images = fn.gaussian_blur(images, window_size=31, sigma=1.5)
        images = fn.laplacian(images, device="gpu")



        angle = fn.random.uniform(range=[-10.0, 10.0])  # small 3D rotation
        matrix = fn.transforms.rotation(angle=angle, axis=[1.0, 0.0, 1.0])  # rotate in XY
        images = images.gpu()
        labels = labels.gpu()
...
def get_data_loaders(flags, num_shards, global_rank):
    ...
    # Modified iterator configuration
    dali_train_iter = DALIGenericIterator(
        pipelines=train_pipe,
        size=-1,  # Let DALI auto-calculate size
        output_map=output_map,
        reader_name="reader",  # Important for sharding
        auto_reset=False,
